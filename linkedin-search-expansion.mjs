...
export async function processLinkedInTasks({
  rootDir = ROOT, receiverResult, expandTask = defaultExpand, evaluateJob = defaultEvaluate, history,
} = {}) {
  ...
  const summary = { tasks_received: files.length, tasks_completed: 0, jobs_seen: 0, jobs_evaluated: 0, jobs_queued: 0, blockers: receiverBlockers };
  const queue = [];
  const seenThisRun = [];
  for (const filename of files) {
    ...
    atomicJson(statePath, { status: 'completed', task_id: task.task_id, identity, exact_url: task.linkedin_search.url, completed_at: new Date().toISOString(), results });
  }
  return { status: summary.blockers.length ? 'blocked' : 'completed', queue, summary };
}
